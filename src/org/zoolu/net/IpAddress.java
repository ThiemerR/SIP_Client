				...
				for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements();) {
					...
					if (!inetAddress.isLoopbackAddress()) { 
						if (inetAddress.getHostAddress().toString().contains(":"))
							continue;
						if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings_Fragment.PREF_STUN, Settings_Fragment.DEFAULT_STUN)) {
							localIpAddress = inetAddress.getHostAddress().toString();
						} else {
							try {
								String StunServer = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_STUN_SERVER, Settings_Fragment.DEFAULT_STUN_SERVER);
								int StunServerPort = Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_STUN_SERVER_PORT, Settings_Fragment.DEFAULT_STUN_SERVER_PORT));
