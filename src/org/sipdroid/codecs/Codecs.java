 ...
 * 
 * Author: Robert Thieme Edit: Delete static class CodecSettings
 */
...
//			add(new SILK8());
	...
	static {
		...
		String prefs = sp.getString(Settings_Fragment.PREF_CODECS, Settings_Fragment.DEFAULT_CODECS);
		if (prefs == null) {
			String v = "";
			...
			for (Codec c : codecs)
				v = v + c.number() + " ";
			e.putString(Settings_Fragment.PREF_CODECS, v);
			e.commit();
		} else {
	...
	private static void addPreferences(PreferenceScreen ps) {
		...
		for(Codec c : codecs) {
			...
			if (c.number() == 9)
				if (ps.getSharedPreferences().getString(Settings_Fragment.PREF_SERVER, Settings_Fragment.DEFAULT_SERVER).equals(Settings_Fragment.DEFAULT_SERVER))
