...
public class SipdroidEngine implements RegisterAgentListener {
	...
	UserAgentProfile getUserAgentProfile(String suffix) {
		...
		user_profile.username = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_USERNAME+suffix, Settings_Fragment.DEFAULT_USERNAME); // modified
		user_profile.passwd = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_PASSWORD+suffix, Settings_Fragment.DEFAULT_PASSWORD);
		if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_DOMAIN+suffix, Settings_Fragment.DEFAULT_DOMAIN).length() == 0) {
			user_profile.realm = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_SERVER+suffix, Settings_Fragment.DEFAULT_SERVER);
		} else {
			user_profile.realm = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_DOMAIN+suffix, Settings_Fragment.DEFAULT_DOMAIN);
		}
		user_profile.realm_orig = user_profile.realm;
		if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_FROMUSER+suffix, Settings_Fragment.DEFAULT_FROMUSER).length() == 0) {
			user_profile.from_url = user_profile.username;
		} else {
			user_profile.from_url = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_FROMUSER+suffix, Settings_Fragment.DEFAULT_FROMUSER);
		}
		
		// MMTel configuration (added by mandrajg)
		user_profile.qvalue = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_MMTEL_QVALUE, Settings_Fragment.DEFAULT_MMTEL_QVALUE);
		user_profile.mmtel = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings_Fragment.PREF_MMTEL, Settings_Fragment.DEFAULT_MMTEL);

		user_profile.pub = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings_Fragment.PREF_EDGE+suffix, Settings_Fragment.DEFAULT_EDGE) ||
			PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings_Fragment.PREF_3G+suffix, Settings_Fragment.DEFAULT_3G);
		return user_profile;
	}
	...
	public boolean StartEngine() {
			...
			if (wl == null) {
				if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).contains(com.sip_client.ui.Settings_Fragment.PREF_KEEPON)) {
					Editor edit = PreferenceManager.getDefaultSharedPreferences(getUIContext()).edit();
	
					edit.putBoolean(com.sip_client.ui.Settings_Fragment.PREF_KEEPON, true);
					edit.commit();
				}
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					...
					if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(com.sip_client.ui.Settings_Fragment.PREF_KEEPON, com.sip_client.ui.Settings_Fragment.DEFAULT_KEEPON)) {
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua.base";
					SipStack.max_retransmission_timeout = 4000;
					SipStack.default_transport_protocols = new String[1];
					SipStack.default_transport_protocols[0] = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_PROTOCOL+(i!=0?i:""),
							user_profile.realm.equals(Settings_Fragment.DEFAULT_SERVER)?"tcp":"udp");
					
					if (SipStack.default_transport_protocols[0].equals("tls"))
						SipStack.default_transport_protocols[0] = "tcp";
					String version = "Sipdroid/" + MainActivity.getVersion() + "/" + Build.MODEL;
	...
	void setOutboundProxy(SipProvider sip_provider,int i) {
		try {
			String host = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_PROTOCOL+(i!=0?i:""), Settings_Fragment.DEFAULT_PROTOCOL).equals("tls")?
					PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_SERVER+(i!=0?i:""), Settings_Fragment.DEFAULT_SERVER):null;
			if (host != null && host.equals(Settings_Fragment.DEFAULT_SERVER))
				host = "www1.pbxes.com";	
			if (sip_provider != null) sip_provider.setOutboundProxy(new SocketAddress(
					IpAddress.getByName(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_DNS+i, Settings_Fragment.DEFAULT_DNS)),
					Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_PORT+(i!=0?i:""), Settings_Fragment.DEFAULT_PORT))),
					host);
		} catch (Exception e) {
		}
	}
	...
	public void updateDNS() {
		...
		for (SipProvider sip_provider : sip_providers) {
			try {
				edit.putString(Settings_Fragment.PREF_DNS+i, IpAddress.getByName(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_SERVER+(i!=0?i:""), "")).toString());
	...
	public boolean call(String target_url,boolean force) {
		...
		if (!found || (ua = uas[p]) == null) {
			if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings_Fragment.PREF_CALLBACK, Settings_Fragment.DEFAULT_CALLBACK) &&
					PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings_Fragment.PREF_POSURL, Settings_Fragment.DEFAULT_POSURL).length() > 0) {
