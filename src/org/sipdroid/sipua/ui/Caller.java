...
public class Caller extends BroadcastReceiver {
		...
		public void onReceive(final Context context, Intent intent) {
	        ...
	        {
        		if (!MainActivity.release) Log.i("SipUA:","outgoing call");
        		if (!MainActivity.on(context)) return;
    			boolean sip_type = !PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PREF, Settings_Fragment.DEFAULT_PREF).equals(Settings_Fragment.VAL_PREF_PSTN);
    	        boolean ask = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PREF, Settings_Fragment.DEFAULT_PREF).equals(Settings_Fragment.VAL_PREF_ASK);
				...
				if (sip_type && !force) {
	    			String sExPat = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_EXCLUDEPAT, Settings_Fragment.DEFAULT_EXCLUDEPAT); 
    			...
    			else 
    			{
	        		if (number != null && !intent.getBooleanExtra("android.phone.extra.ALREADY_CALLED",false)) {
	        		    	...
	        		    	if (sp.contains("prefix")) {
	        		    	    String prefix = sp.getString(Settings_Fragment.PREF_PREFIX, Settings_Fragment.DEFAULT_PREFIX);
	        		    	    Editor editor = sp.edit();
	        		    	    if (!prefix.trim().equals("")) {
	        		    		editor.putString(Settings_Fragment.PREF_SEARCH, "(.*)," + prefix + "\\1");
	        		    	    }
	        		    	    editor.remove(Settings_Fragment.PREF_PREFIX);
	        		    	    editor.commit();
	        		    	}
						...
						if (!ask && !force && PreferenceManager.getDefaultSharedPreferences(context).getBoolean(Settings_Fragment.PREF_PAR, Settings_Fragment.DEFAULT_PAR)) 
	    				{
						...
						if (PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PREF, Settings_Fragment.DEFAULT_PREF).equals(Settings_Fragment.VAL_PREF_SIPONLY))
							force = true;
	    				if (!ask && Receiver.engine(context).call(number,force))
	    					setResultData(null);
	    				else if (!ask && PreferenceManager.getDefaultSharedPreferences(context).getBoolean(Settings_Fragment.PREF_CALLTHRU, Settings_Fragment.DEFAULT_CALLTHRU) &&
	    						(callthru_prefix = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_CALLTHRU2, Settings_Fragment.DEFAULT_CALLTHRU2)).length() > 0) {
		...
		static private String searchReplaceNumber(Context context,String number) {
	    	SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
			String pattern = sp.getString(Settings_Fragment.PREF_SEARCH, Settings_Fragment.DEFAULT_SEARCH);
