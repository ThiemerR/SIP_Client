...
import org.sipdroid.sipua.base.RegisterAgent;
	...
	public static String isPossible(Context context) {
		...
		email = trunkserver = null;
	   	for (int i = 0; i < SipdroidEngine.LINES; i++) {
	   		String j = (i!=0?""+i:"");
	   		String username = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_USERNAME+j, Settings_Fragment.DEFAULT_USERNAME),
	   			server = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_SERVER+j, Settings_Fragment.DEFAULT_SERVER);
	   		if (username.equals("") || server.equals(""))
	   			continue;
	   		if (server.contains("pbxes"))
	   			found = true;
	   		else if (i == 0 &&
	   				!PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PROTOCOL+j, Settings_Fragment.DEFAULT_PROTOCOL).equals("tcp") &&
	   				PreferenceManager.getDefaultSharedPreferences(context).getBoolean(Settings_Fragment.PREF_3G+j, Settings_Fragment.DEFAULT_3G) &&
	   				Receiver.engine(context).isRegistered(i) &&
	   				Receiver.engine(context).ras[i].CurrentState == RegisterAgent.REGISTERED) {
	   			trunkserver = server;
	   			trunkuser = username;
	   			trunkpassword = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PASSWORD+j, Settings_Fragment.DEFAULT_PASSWORD);
	   			trunkport = PreferenceManager.getDefaultSharedPreferences(context).getString(Settings_Fragment.PREF_PORT+j, Settings_Fragment.DEFAULT_PORT);
	   		}
	   	}
				...
				try {
					...
					if (line != null) {
						if (line.equals("OK")) {
							...
							edit.putString(Settings_Fragment.PREF_SERVER, Settings_Fragment.DEFAULT_SERVER);
							edit.putString(Settings_Fragment.PREF_USERNAME, etName.getText()+"-200");
							edit.putString(Settings_Fragment.PREF_DOMAIN, Settings_Fragment.DEFAULT_DOMAIN);
							edit.putString(Settings_Fragment.PREF_FROMUSER, Settings_Fragment.DEFAULT_FROMUSER);
							edit.putString(Settings_Fragment.PREF_PORT, "5061");
							edit.putString(Settings_Fragment.PREF_PROTOCOL, "tcp");
							edit.putString(Settings_Fragment.PREF_PASSWORD, password);
							edit.commit();
